...
export default function ListPage() {
  ...
  const [brandsBU, updateBradsBU] = useState([]);
  ...
  const getBrands = async () => {
    ...
    await axios
      .get("/api/brands")
      .then((response) => {
        updateBrands(response.data);
        updateBradsBU(response.data);
      })
      .catch((err) => {
        console.log(err);
      })
  ...
  const brandsList = paginate(brands, currentPage, pageSize);
  return (
    <div className="p-6 w-full flex flex-col max-h-[91vh] gap-4">
      <Toaster position="bottom-right" />
      <TitleList
        title={"Listado de marcas"}
        btnTitle={"Agregar marca"}
        btnLink={"/marcas/create"}
      />
